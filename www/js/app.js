...
var api = 'http://qsit.com.br/sibac/api/';
var useAuth = true;
  ...
  $ionicPlatform.ready(function() {
    ...
    $rootScope.$on('$cordovaNetwork:offline', function(event, networkState){      
      if(showOfflineMessage && blockIfOffline){
        $ionicLoading.show({
          template: 'Seu dispositivo esta offline.'
        }); 
      }
    ...
    function initPushwoosh() {
      ...
      pushNotification.registerDevice(
        ...
        function(status) {
          console.warn('Falha ao registrar : ' + JSON.stringify(status));
  ...
  $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState) {        
    ...
    if(isGoingToLogin && User.isLoggedIn()){
      ...
      if( showWalkthrough && !Walkthrough.hasBeenShown() ){
        console.log('Indo para o passo a passo');
    ...
    if (requireLogin) {
      ...
      if( showWalkthrough && !Walkthrough.hasBeenShown() ){
        console.log('Indo para pagina de apresentação');
        $state.go("walkthrough");      
      } else {
        console.log('Indo para Home');
        $state.go("auth.index");  
